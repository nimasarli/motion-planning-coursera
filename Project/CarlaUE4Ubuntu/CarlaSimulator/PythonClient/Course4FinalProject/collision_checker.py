    ...
    def select_best_path_index(self, paths, collision_check_array, goal_state):
        ...
        for i in range(len(paths)):
            ...
            if collision_check_array[i]:
                ...
                # --------------------------------------------------------------
                # score = ...
                score = sqrt((paths[i][0][-1] - goal_state[0])**2 + \
                             (paths[i][1][-1] - goal_state[1])**2)  
                # --------------------------------------------------------------
                    ...
                    if j == i:
                        ...
                        if not collision_check_array[j]:
                            ...
                            dists = scipy.spatial.distance.cdist( \
                                        np.array(list(zip(paths[i][0], 
                                                          paths[i][1]))), \
                                        np.array(list(zip(paths[j][0], 
                                                          paths[j][1]))))
                            min_dist = np.min(dists)
                            if min_dist > 0:
                                score += self._weight / min_dist
                            else:
                                score += float('Inf')
                            # --------------------------------------------------
